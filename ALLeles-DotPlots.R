...
#dotchart(AFLP$Cen[as.logical(match(AFLP$Fungicide, "Boscalid", nomatch = F))], labels=AFLP$IsolateID[as.logical(match(AFLP$Fungicide, "Boscalid"))])
bos_1 <- AFLP$Cen[as.logical(match(AFLP$Fungicide, "Boscalid", nomatch = F))]
bos_2 <- AFLP$Cen2[as.logical(match(AFLP$Fungicide, "Boscalid", nomatch = F))]
az_1 <- AFLP$Cen[as.logical(match(AFLP$Fungicide, "Azoxystrobin", nomatch = F))]
az_2 <- AFLP$Cen2[as.logical(match(AFLP$Fungicide, "Azoxystrobin", nomatch = F))]
ip_1 <- AFLP$Cen[as.logical(match(AFLP$Fungicide, "Iprodione", nomatch = F))]
ip_2 <- AFLP$Cen2[as.logical(match(AFLP$Fungicide, "Iprodione", nomatch = F))]
py_1 <- AFLP$Cen[as.logical(match(AFLP$Fungicide, "Pyraclostrobin", nomatch = F))]
py_2 <- AFLP$Cen2[as.logical(match(AFLP$Fungicide, "Pyraclostrobin", nomatch = F))]
tm_1 <- AFLP$Cen[as.logical(match(AFLP$Fungicide, "ThiophanateMethyl", nomatch = F))]
tm_2 <- AFLP$Cen2[as.logical(match(AFLP$Fungicide, "ThiophanateMethyl", nomatch = F))]
...
plot(jitter(rep(0,9),amount=0.15), py_1, pch=21, col=1,
     xlim=range(-0,9.5), ylim=range(-1,43),
...
title(main="Alleles of Censored AFLP by Fungicide")

##Add in the legend
legend(0, 41, c("Experiment 1", "Experiment 2"), col=1:2, pch=1)
