...
SSR <- read.csv("~/Documents/people/sajeewa amaradasa/sublethal paper/sublethalstressmanuscript/SSR2.csv")

dotchart(SSR$SSR[as.logical(match(SSR$Fungicide, "Boscalid", nomatch = F))], labels=SSR$IsolateID[as.logical(match(SSR$Fungicide, "Boscalid"))])
bos_1 <- SSR$SSR[as.logical(match(SSR$Fungicide, "Boscalid", nomatch = F))]
bos_2 <- SSR$SSR2[as.logical(match(SSR$Fungicide, "Boscalid", nomatch = F))]
az_1 <- SSR$SSR[as.logical(match(SSR$Fungicide, "Azoxystrobin", nomatch = F))]
az_2 <- SSR$SSR2[as.logical(match(SSR$Fungicide, "Azoxystrobin", nomatch = F))]
ip_1 <- SSR$SSR[as.logical(match(SSR$Fungicide, "Iprodione", nomatch = F))]
ip_2 <- SSR$SSR2[as.logical(match(SSR$Fungicide, "Iprodione", nomatch = F))]
py_1 <- SSR$SSR[as.logical(match(SSR$Fungicide, "Pyraclostrobin", nomatch = F))]
py_2 <- SSR$SSR2[as.logical(match(SSR$Fungicide, "Pyraclostrobin", nomatch = F))]
tm_1 <- SSR$SSR[as.logical(match(SSR$Fungicide, "ThiophanateMethyl", nomatch = F))]
tm_2 <- SSR$SSR2[as.logical(match(SSR$Fungicide, "ThiophanateMethyl", nomatch = F))]
...
plot(jitter(rep(0,9),amount=0.15), py_1, pch=21, col=1,
     xlim=range(-0,9.5), ylim=range(-0.5,27.5),
...
title(main="Change in SSR by Fungicide")

##Add in the legend
legend(6.8, 27, c("Experiment 1", "Experiment 2"), col=1:2, pch=1)
